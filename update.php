...
if (isset($_SESSION['key'])) {
    if (@$_GET['demail'] && $_SESSION['key'] == 'admin') {
        $demail = @$_GET['demail'];
        $r1 = mysqli_query($con, "DELETE FROM `rank` WHERE email='$demail' ") or die('Error1');
        $r2 = mysqli_query($con, "DELETE FROM history WHERE email='$demail' ") or die('Error2');
        $result = mysqli_query($con, "DELETE FROM student WHERE email='$demail' ") or die('Error3');
...
if (isset($_SESSION['key'])) {
    if (@$_GET['q'] == 'addquiz' && $_SESSION['key'] == 'admin') {
        ...
        $time = $_POST['time'];
        $id = uniqid();
        $q3 = mysqli_query($con, "INSERT INTO quiz VALUES  ('$id','$name' , '$sahi' , '$wrong','$subject','$total', NOW())");
        header("location:dashboard.php?q=4&step=2&eid=$id&n=$total");
    }
}
//هنا معلق
if (isset($_SESSION['key'])) {
    if (@$_GET['q'] == 'addsubject' && $_SESSION['key'] == 'admin') {
        // $id=uniqid();
        //        $Subject_id = $_POST['Subject_id'];
        //        $name = ucwords(strtolower($name));
        //        $name = $_POST['name'];
        //        $hours = $_POST['hours'];
        //        $q3 = mysqli_query($con, "INSERT INTO subjects VALUES  ('$Subject_id' , '$name' , '$hours',NOW())") or die('Error189');
        //        header("location:dashboard.php?q=6");


        $Subject_id = ucwords(strtolower(@$_POST['Subject_id']));
        $name = @$_POST['name'];
        $hours = @$_POST['hours'];
        mysqli_query($con, "INSERT INTO subjects (`subject_id`, `name`, `num_houre`) VALUES  ('$Subject_id','$name','$hours')") or die('Error1223');

        header("location:dashboard.php?q=3");
    }
}
if (isset($_SESSION['key'])) {
    if (@$_GET['q'] == 'editsubject' && $_SESSION['key'] == 'admin') {

        $subject_id = $_GET['dsubject_id'];
        $Subject_id = ucwords(strtolower(@$_POST['Subject_id']));
        $name = @$_POST['name'];
        $hours = @$_POST['hours'];
        mysqli_query($con, "UPDATE `subjects` SET `subject_id`='$Subject_id',`name`='$name',`num_houre`='$hours' WHERE subject_id = '$subject_id'") or die('Error1224');

        header("location:dashboard.php?q=3");
    }
}
if (isset($_SESSION['key'])) {
    if (@$_GET['q'] == 'deletesubject' && $_SESSION['key'] == 'admin') {

        $subject_id = @$_GET['dsubject_id'];
        mysqli_query($con, "DELETE FROM `subjects` WHERE subject_id = '$subject_id'") or die('Error1225');
